...
impl AsGlUniform for crate::math::V2<f32> {
    fn bind(&self, gl: &GL, program: &GlProgram, location: Option<&WebGlUniformLocation>) {
        [self.x, self.y].bind(gl, program, location);
    }
}

impl AsGlUniform for crate::math::V4<f32> {
    fn bind(&self, gl: &GL, program: &GlProgram, location: Option<&WebGlUniformLocation>) {
        [self.x, self.y, self.z, self.w].bind(gl, program, location);
    }
}

impl AsGlUniform for crate::math::M3<f32> {
    fn bind(&self, gl: &GL, program: &GlProgram, location: Option<&WebGlUniformLocation>) {
        [
            self.c0.x, self.c0.y, self.c0.z, self.c1.x, self.c1.y, self.c1.z, self.c2.x, self.c2.y,
            self.c2.z,
        ]
        .bind(gl, program, location);
    }
}
...
impl<V: AsGlVertex> GlModel<V> {
    ...
    pub fn empty(gl: Rc<GlContext>) -> GlModel<V> {
...
impl GlTexture {
    pub fn new(gl: Rc<GlContext>, width: u32, height: u32, pixel_format: PixelFormat) -> GlTexture {
...
impl GlFrameBuffer {
    pub fn new(gl: Rc<GlContext>, width: u32, height: u32) -> GlFrameBuffer {
        let texture = GlTexture::new(gl.clone(), width, height, PixelFormat::RGBA);
